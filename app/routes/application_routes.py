...
from ..config import settings  # use settings directly
...
def create_application(
  ...
  # NEW: prevent duplicate application for same job by the same user
  existing = (
  ...
  try:
      ...
      logger.error(
          f"Application created successfully (ID: {db_application.id}) but email failed"
      )
...
def download_cv(
  ...
  # Check authorization
  if (
      current_user.role != models.UserRole.HIRING_MANAGER
  ...
  if not application.cv_content:
      raise HTTPException(
          status_code=404, detail="No CV uploaded for this application"
      )
...
def update_application_status(
  ...
  Expected values for new_status: "pending", "shortlisted", "rejected", "interview".
  Only hiring managers can change this.
  """
  application = db.get(models.Application, application_id)
  if not application:
      raise HTTPException(status_code=404, detail="Application not found")
  ...
  valid_status = {"pending", "shortlisted", "rejected", "interview"}
